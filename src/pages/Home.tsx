...
import {
  IconMacroSystem,
} from '../constants/homeProductPointIcons'
...
const TEKS_RAY_DALIO = {
  ...
  description: 'Basados en los principios de Ray Dalio, entendemos que la economía funciona como una máquina simple regida por ciclos de deuda a corto y largo plazo. No navegamos estos ciclos con especulación, sino con precisión arquitectónica y geométrica.',
}
...
const RAY_DALIO_ASSETS = [
  ...
  { type: 'quote', text: 'Principles are fundamental truths that serve as the foundations for behavior.' },
]
...
export default function Home() {
  ...
  const beats: any[] = [
    ...
    { ...TEKS_4_SERVICES, align: 'ml-auto items-end text-right justify-center pr-6 lg:pr-16' }, 
  ...
  const timeline = [0, 0.10, 0.60, 0.62, 0.75, 0.77, 0.92, 0.95]
  ...
  const canvasOpacity = useTransform(scrollYProgress, [0.22, 0.25, 0.98, 0.99], [1, 0, 0, 1])
  const canvasZ = useTransform(scrollYProgress, [0.97, 0.98], [0, 26])

  const t4HeaderOp = useTransform(scrollYProgress, [0.60, 0.61, 0.64, 0.65], [0, 1, 1, 0])
  const t4HeaderY = useTransform(scrollYProgress, [0.60, 0.65], [50, -50])
  ...
  const dalioDraw = useTransform(scrollYProgress, [0.77, 0.80, 0.82, 0.84], [0, 1, 1, 0])
  ...
  const dTime = [0, 0.10, 0.20, 0.24, 0.28, 0.60, 0.62, 0.75, 0.77, 0.84, 0.86, 0.985, 1]
  ...
  const diamondZRaw = useTransform(
    scrollYProgress, 
    [0, 0.199, 0.20, 0.629, 0.63, 1], 
    [50, 50, 0, 0, 50, 50]
  )
  ...
  const insideDiamondOpacity = useTransform(scrollYProgress, [0.25, 0.28, 0.60, 0.62], [0, 1, 1, 0])
  const insideDiamondScale = useTransform(scrollYProgress, [0.25, 0.62], [0.8, 1.2])
  const globalGridOpacity = useTransform(scrollYProgress, [0.60, 0.62, 0.94, 0.96], [0.05, 0, 0, 0.05])
  const title3Opacity = useTransform(scrollYProgress, [0.20, 0.23, 0.26], [1, 1, 0])
  ...
  const heroStrokeOp = useTransform(scrollYProgress, [0.08, 0.10], [1, 0])
  ...
  const b1StrokeOp = useTransform(scrollYProgress, [0.10, 0.11, 0.19, 0.20], [0, 1, 1, 0])
  const b1FillOp = useTransform(scrollYProgress, [0.17, 0.19, 0.19, 0.20], [0, 1, 1, 0])

  const b2StrokeDashoffset = useTransform(scrollYProgress, [0.20, 0.23, 0.24, 0.28], [1000, 0, 0, 1000])
  const b2PathLength = useTransform(scrollYProgress, [0.20, 0.23, 0.24, 0.28], [0, 1, 1, 0])
  const b2StrokeOp = useTransform(scrollYProgress, [0.19, 0.20, 0.28, 0.29], [0, 1, 1, 0])
  const b2FillOp = useTransform(scrollYProgress, [0.22, 0.23, 0.24, 0.25], [0, 1, 1, 0])

  const b0Y = useTransform(scrollYProgress, [0.08, 0.10], [0, -150]) 
  const b1Y = useTransform(scrollYProgress, [0.18, 0.20], [0, -150]) 
  const b2Y = useTransform(scrollYProgress, [0.58, 0.60], [0, -150]) 

  useEffect(() => {
    const unsub = scrollYProgress.on('change', value => {
      if (isManualScroll.current) return
      if (value < 0.10) setActiveBeat(0)
      else if (value < 0.20) setActiveBeat(1)
      else if (value < 0.60) setActiveBeat(2) 
  ...
  useEffect(() => {
    ...
    const particles: { x: number; y: number; vx: number; vy: number; size: number; angle: number; spin: number; shape: string }[] = []; 
    ...
    const draw = () => {
      ...
      for (const p of particles) {
        ...
        if (p.x < -30) p.x = canvas.width + 30; if (p.x > canvas.width + 30) p.x = -30;
        if (p.y < -30) p.y = canvas.height + 30; if (p.y > canvas.height + 30) p.y = -30;
      }
      ctx.strokeStyle = NAVY; ctx.lineWidth = 0.8; ctx.lineCap = 'round'; ctx.lineJoin = 'round';

      for (const p of particles) { 
        ctx.globalAlpha = 0.4; ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(p.angle); ctx.beginPath();
        const s = p.size / 2;
        switch(p.shape) {
          case 'diamond': ctx.moveTo(0, -s); ctx.lineTo(s * 0.6, 0); ctx.lineTo(0, s); ctx.lineTo(-s * 0.6, 0); ctx.closePath(); break;
          case 'cross': ctx.moveTo(-s * 0.8, -s * 0.8); ctx.lineTo(s * 0.8, s * 0.8); ctx.moveTo(s * 0.8, -s * 0.8); ctx.lineTo(-s * 0.8, s * 0.8); break;
          case 'triangle': ctx.moveTo(0, -s); ctx.lineTo(s * 0.8, s * 0.6); ctx.lineTo(-s * 0.8, s * 0.6); ctx.closePath(); break;
          case 'lines': ctx.moveTo(-s * 0.5, -s * 0.8); ctx.lineTo(s * 0.5, -s * 0.4); ctx.moveTo(-s * 0.5, -s * 0.2); ctx.lineTo(s * 0.5, s * 0.2); ctx.moveTo(-s * 0.5, s * 0.4); ctx.lineTo(s * 0.5, s * 0.8); break;
          case 'facet': ctx.moveTo(0, -s); ctx.lineTo(s * 0.6, 0); ctx.lineTo(0, s); break;
        }
  ...
  // ── ZOOM LOCK STYLE ──
  // Saat browser zoom 200%: wrapper diperbesar 200% lalu di-zoom 0.5
  // Hasil visual = 200% * 0.5 = 100% (identik dengan tampilan normal)
  ...
  return (
    <PageTransition>
      <motion.main style={{ backgroundColor, color: textColor }} className="relative w-full transition-colors duration-500">

        <div ref={storyRef} className="relative h-[10000vh] w-full">

          {/* Sticky container: overflow-hidden dihapus saat zoomed agar wrapper besar tidak terpotong */}
          <div className={`sticky top-0 h-screen w-full ${isZoomed ? '' : 'overflow-hidden'}`}>

            {/* Zoom Lock Wrapper: menutupi seluruh sticky area */}
            <div style={zoomWrapperStyle as any}>

              <motion.canvas ref={canvasRef} style={{ opacity: canvasOpacity, zIndex: canvasZ }} className="absolute inset-0 pointer-events-none" />
              <motion.div style={{ opacity: globalGridOpacity }} className="pointer-events-none absolute inset-0 z-0 [background-image:linear-gradient(currentColor_1px,transparent_1px),linear-gradient(90deg,currentColor_1px,transparent_1px)] [background-size:72px_72px]" />
              <motion.div style={{ opacity: insideDiamondOpacity, scale: insideDiamondScale, backgroundImage: `repeating-linear-gradient(60deg, transparent, transparent 30px, rgba(3,0,53,0.06) 30px, rgba(3,0,53,0.06) 32px), repeating-linear-gradient(-60deg, transparent, transparent 30px, rgba(3,0,53,0.06) 30px, rgba(3,0,53,0.06) 32px)`, backgroundColor: '#ffffff' }} className="absolute inset-[-50%] pointer-events-none z-10" />

              <motion.div style={{ opacity: t4HeaderOp, y: t4HeaderY }} className="absolute inset-0 flex items-center justify-end px-4 sm:px-6 lg:px-24 pointer-events-none z-30">
                <div className="w-full max-w-[80vw] md:max-w-[500px] lg:max-w-[600px] text-right flex flex-col items-end">
                  <div className="mb-6 inline-flex items-center gap-3 rounded-full border border-[#030035]/10 bg-[#030035]/5 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.45em] backdrop-blur-xl"><span className="h-1.5 w-1.5 rounded-full bg-[#E5997B]" />{TEKS_4_SERVICES.eyebrow}</div>
                  <h1 className="font-display text-4xl sm:text-5xl md:text-7xl lg:text-[6.5rem] leading-[0.9] tracking-[-0.04em] text-[#030035]">{TEKS_4_SERVICES.title}</h1>
                  <p className="mt-4 md:mt-6 text-sm sm:text-base leading-relaxed opacity-75 md:text-lg max-w-md text-[#030035]">{TEKS_4_SERVICES.description}</p>
                </div>
              </motion.div>

              {/* ── BERLIAN 3D ── */}
              <motion.div style={{ x: diamondX, y: diamondY, zIndex: diamondZ }} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <motion.div style={{ scale: diamondScale, rotateZ: diamondRotate }} className="absolute origin-center pointer-events-auto cursor-pointer flex justify-center items-center" onClick={() => handlePointClick(1)}>
                  <div className="w-[36rem] h-[36rem] max-w-[70vw] max-h-[70vh] aspect-square drop-shadow-2xl flex justify-center items-center"><DimaDiamond3D /></div>
                </motion.div>
                <motion.div style={{ y: fig1Y, scale: diamondScale, rotateZ: diamondRotate, opacity: figOpacity }} className="absolute origin-center pointer-events-auto cursor-pointer flex justify-center items-center" onClick={() => handlePointClick(0)}>
                  <div className="w-[36rem] h-[36rem] max-w-[70vw] max-h-[70vh] aspect-square drop-shadow-2xl flex justify-center items-center"><DimaDiamond3D /></div>
                </motion.div>
                <motion.div style={{ y: fig2Y, scale: diamondScale, rotateZ: diamondRotate, opacity: figOpacity }} className="absolute origin-center pointer-events-auto cursor-pointer flex justify-center items-center" onClick={() => handlePointClick(2)}>
                  <div className="w-[36rem] h-[36rem] max-w-[70vw] max-h-[70vh] aspect-square drop-shadow-2xl flex justify-center items-center"><DimaDiamond3D /></div>
                </motion.div>
              </motion.div>
              ...
              <div className="absolute inset-0 pointer-events-none z-30 hidden md:block">
                {products.map((product, idx) => (<ProductSequenceItem key={product.name} product={product} index={idx} total={products.length} scrollYProgress={scrollYProgress} />))}
              </div>

              {/* ── PRODUCTS MOBILE SIMPLIFIED ── */}
              <motion.div style={{ opacity: title3Opacity }} className="absolute inset-0 flex items-center justify-center z-20 md:hidden px-6">
                <div className="text-center max-w-sm">
                  <h2 className="font-display text-3xl leading-tight tracking-tight text-[#030035] mb-4">{TEKS_3_PRODUCTS.title}</h2>
                  <p className="text-base text-[#030035]/70 leading-relaxed">{TEKS_3_PRODUCTS.description}</p>
                </div>
              </motion.div>

              {/* ── SERVICES DESKTOP (Complex Layout) ── */}
              <div className="hidden lg:block">
                <div className="absolute top-1/2 left-[30vw] lg:left-[25vw] w-full max-w-[400px] md:max-w-[450px] pointer-events-none z-30 -translate-y-1/2">
                  {services.map((service, idx) => {
                    ...
                    return (
                      <motion.div key={idx} style={{ y: serviceOffsets[idx], opacity: servicesOp }} className="absolute top-1/2 left-0 w-full -translate-y-1/2 pointer-events-auto">
                        <div onClick={() => handlePointClick(idx)} className="relative cursor-pointer group p-4 transition-all duration-500">
                          <motion.div initial={false} animate={{ opacity: isOpen ? 0.3 : 0 }} transition={{ duration: 0.8 }} className="absolute inset-0 bg-[#ffffff] blur-[50px] rounded-full pointer-events-none" />
                          <h3 className={`relative z-10 text-3xl md:text-5xl font-display font-medium tracking-tight leading-none transition-all duration-500 ${isOpen ? 'text-[#030035] opacity-100 scale-105' : 'text-transparent opacity-40 hover:opacity-100'}`} style={{ WebkitTextStroke: isOpen ? '0px' : '1.5px #030035' }}>{service.name}</h3>
                          <p className={`relative z-10 font-sans font-bold text-xs tracking-[0.2em] mt-3 transition-all duration-500 ${isOpen ? 'opacity-100 text-[#030035]' : 'opacity-0 text-[#030035]'}`}>0{service.number} — SECTION</p>
                        </div>
                      </motion.div>
                    )
                  })}
                ...
                <motion.div style={{ opacity: servicesOp }} className="absolute inset-0 w-full h-full pointer-events-none z-20">
                  ...
                  <div className="absolute top-1/2 right-[5vw] lg:right-[8vw] w-full max-w-[600px] lg:max-w-[700px] -translate-y-1/2 pointer-events-auto">
                    <AnimatePresence mode="wait">
                      {openIndex !== null && services[openIndex] && (
                        <motion.div key={openIndex} initial={{ opacity: 0, y: 15, filter: 'blur(8px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, y: -15, filter: 'blur(8px)' }} transition={{ duration: 0.4, ease: "easeOut" }} className="flex flex-col text-left">
                          <div className="relative w-full p-8 md:p-12 rounded-[1.5rem] border border-[#030035]/10 bg-white/20 backdrop-blur-md shadow-[0_20px_40px_rgba(3,0,53,0.05)] overflow-hidden">
                            <div className="absolute top-4 left-4 w-4 h-4 border-t border-l border-[#030035]/30" /><div className="absolute top-4 right-4 w-4 h-4 border-t border-r border-[#030035]/30" />
                            <div className="absolute bottom-4 left-4 w-4 h-4 border-b border-l border-[#030035]/30" /><div className="absolute bottom-4 right-4 w-4 h-4 border-b border-r border-[#030035]/30" />
                            <div className="flex items-center gap-4 mb-8 justify-center"><span className="h-[1px] flex-1 bg-[#030035]/20" /><span className="text-[10px] font-bold tracking-[0.3em] text-[#030035] uppercase">Capítulo 0{openIndex + 1}</span><span className="h-[1px] flex-1 bg-[#030035]/20" /></div>
                            <div className="w-full flex justify-center mb-8 relative z-10">
                              {openIndex === 0 && <ServiceIconStructuration isOpen={true} />}{openIndex === 1 && <ServiceIconRisk isOpen={true} />}{openIndex === 2 && <ServiceIconSacredGeometry isOpen={true} />}
                            </div>
                            <div className="relative z-10"><p className="text-lg md:text-xl lg:text-2xl leading-relaxed text-[#030035] font-serif font-medium"><span className="float-left text-[3.5rem] md:text-[4rem] leading-[0.8] mr-3 mt-1 text-[#030035] font-display opacity-90 drop-shadow-sm">{services[openIndex].description.charAt(0)}</span>{services[openIndex].description.slice(1)}</p></div>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
              ...
              <motion.div style={{ opacity: servicesOp }} className="absolute inset-0 flex items-center justify-center z-30 lg:hidden px-6 py-12">
                <div className="w-full max-w-sm space-y-4">
                  {services.map((service, idx) => (
                    <div key={idx} className="bg-white/70 backdrop-blur-md rounded-xl p-5 border border-[#030035]/10 shadow-sm">
                      <p className="text-[10px] font-bold tracking-[0.2em] text-[#030035]/60 uppercase mb-1">0{service.number} — SECTION</p>
                      <h3 className="text-xl font-display font-medium text-[#030035] mb-2">{service.name}</h3>
                      <p className="text-sm text-[#030035]/80 leading-relaxed">{service.description}</p>
                    </div>
                  ))}
                </div>
              </motion.div>
              ...
              {/* ── RAY DALIO DESKTOP ── */}
              <motion.div style={{ opacity: dalioOp }} className="absolute inset-0 pointer-events-none z-30 hidden md:block">
                 <div className="w-full h-full max-w-[1800px] mx-auto relative px-6 lg:px-24 overflow-hidden">
                   <div className="absolute inset-0 z-10 pointer-events-none">
                     <div className="w-full h-full pointer-events-auto">
                       <RayDalioFilmRoll assets={RAY_DALIO_ASSETS as Array<{ type: 'photo' | 'quote'; src?: string; text?: string }>} filmY={dalioFilmY} />
                     </div>
                   </div>
                   ...
                   <div className="absolute bottom-12 left-[10vw] lg:left-[11vw] w-full max-w-[850px] md:max-w-[950px] lg:max-w-[1100px] xl:max-w-[1250px] flex flex-col items-start text-left pointer-events-auto z-0">
                      ...
                      <motion.div style={{ opacity: dalioOp, borderColor }} className="pointer-events-auto">
                        <div className="mb-4 inline-flex items-center gap-3 rounded-full border border-current/10 bg-current/5 px-4 py-2 text-[10px] sm:text-xs font-semibold uppercase tracking-[0.45em] backdrop-blur-xl">
                          <span className="h-1.5 w-1.5 rounded-full bg-[#E5997B] shadow-[0_0_18px_rgba(229,153,123,0.8)]" />{TEKS_RAY_DALIO.eyebrow}
                        </div>
                        <svg viewBox="0 0 600 180" className="w-full h-auto max-h-[30vh] overflow-visible drop-shadow-sm mb-4 max-w-[500px]">
                          <motion.text x="0" y="40" textAnchor="start" dominantBaseline="hanging" className="font-display" fontSize="75" fontWeight="400" fill="none" stroke="#030035" strokeWidth="2.5" style={{ pathLength: dalioDraw, opacity: dalioOp }}>
                            <tspan x="0" dy="0">La Máquina</tspan><tspan x="0" dy="85">Económica</tspan>
                          </motion.text>
                          <motion.text x="0" y="40" textAnchor="start" dominantBaseline="hanging" className="font-display" fontSize="75" fontWeight="400" fill="#030035" stroke="none" style={{ opacity: dalioFillOp }}>
                            <tspan x="0" dy="0">La Máquina</tspan><tspan x="0" dy="85">Económica</tspan>
                          </motion.text>
                        </svg>
              ...
              <motion.div style={{ opacity: dalioOp }} className="absolute inset-0 flex items-center justify-center z-30 md:hidden px-6">
                <div className="max-w-sm w-full">
                  <div className="mb-3 inline-flex items-center gap-3 rounded-full border border-[#030035]/10 bg-[#030035]/5 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.45em] backdrop-blur-xl">
                    <span className="h-1.5 w-1.5 rounded-full bg-[#E5997B]" />{TEKS_RAY_DALIO.eyebrow}
                  </div>
                  <h2 className="font-display text-3xl leading-tight text-[#030035] mb-4">La Máquina<br/>Económica</h2>
              ...
              <AnimatePresence>
                {activeBeat === 1 && (
                  <motion.div
                    ...
                    style={{ y: b1Y }}
                  >
                     <motion.svg viewBox="0 0 240 240" className="w-full h-full max-h-[60vh] text-[#030035]" style={{ opacity: b1StrokeOp }}>
                       <defs>
                         <pattern id="hatch-b1" width="4" height="4" patternTransform="rotate(45 0 0)" patternUnits="userSpaceOnUse"><line x1="0" y1="0" x2="0" y2="4" stroke="#030035" strokeWidth="0.5" opacity="0.4" /></pattern>
                         <pattern id="hatch-bronze" width="6" height="6" patternTransform="rotate(-45 0 0)" patternUnits="userSpaceOnUse"><line x1="0" y1="0" x2="0" y2="6" stroke="#E5997B" strokeWidth="0.5" opacity="0.5" /></pattern>
                       </defs>
              ...
              <div className="absolute inset-0 flex px-6 sm:px-10 lg:px-16 pointer-events-none z-20">
                <div className="mx-auto flex h-full w-full max-w-[1800px] relative">
                  <AnimatePresence>
                    {(activeBeat < 3 || activeBeat === 6) && (
                      <motion.div key={activeBeat} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -40 }} transition={{ duration: 0.6, ease: 'easeOut' }} className={`absolute inset-0 pointer-events-auto flex w-full flex-col md:max-w-2xl lg:max-w-4xl xl:max-w-5xl ${beats[activeBeat].align}`} style={{ opacity: activeBeat === 2 ? title3Opacity : activeBeat === 6 ? finaleOp : 1, y: activeBeat === 6 ? finaleY : 0 }}>

                        {/* ── FINALE ── */}
                        {activeBeat === 6 ? (
                          <div className="flex flex-col items-center justify-end w-full h-full pb-[10vh] pt-[12vh] text-center relative px-4"> 
                            <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-10 z-0 hidden md:flex">
                            ...
                            <div className="w-full flex flex-col items-center z-10 mt-auto">
                              <motion.div style={{ borderColor }} className="mb-6 inline-flex items-center gap-3 rounded-full border border-current/10 bg-current/5 px-6 py-2.5 text-xs font-semibold uppercase tracking-[0.45em] backdrop-blur-xl relative">
                                <span className="h-1.5 w-1.5 rounded-full bg-[#E5997B] shadow-[0_0_18px_rgba(229,153,123,0.8)]" />EL FUTURO DE TU PATRIMONIO
                              </motion.div>
                              ...
                              <div className="mt-8 flex flex-wrap gap-4 w-full justify-center">
                                <Link to="/contact">
                                  <motion.div style={{ backgroundColor: textColor, color: backgroundColor }} className="inline-flex items-center justify-center rounded-full px-12 py-4 text-base font-semibold transition-transform hover:-translate-y-0.5 shadow-[0_10px_30px_rgba(3,0,53,0.15)] pointer-events-auto">
                                    Agenda una Llamada
                                  </motion.div>
                                </Link>
                                <Link to="/about">
                                  <motion.div style={{ borderColor: textColor }} className="inline-flex items-center justify-center rounded-full border border-[#030035]/20 px-12 py-4 text-base font-semibold backdrop-blur-xl transition-opacity hover:opacity-70 bg-[#030035]/5 text-[#030035] pointer-events-auto">
                                    Conoce Dima
                                  </motion.div>
                                </Link>
                              </div>
                          ...
                          <>
                            {activeBeat !== 5 && (
                              <motion.div style={{ borderColor }} className="mt-2 mb-6 inline-flex items-center gap-3 rounded-full border border-current/10 bg-current/5 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.45em] backdrop-blur-xl z-10 relative">
                                <span className="h-1.5 w-1.5 rounded-full bg-bronze shadow-[0_0_18px_rgba(229,153,123,0.8)]" />{beats[activeBeat]?.eyebrow}
                              </motion.div>
                            )}
                            ...
                            {activeBeat === 0 && (
                              <>
                                <motion.div style={{ y: b0Y }} className="w-full relative mt-2 mb-4 -ml-1 hidden md:block">
                                  <svg viewBox="0 0 800 720" className="w-full h-auto max-h-[75vh] max-w-[100%] md:max-w-[900px] object-contain object-left overflow-visible">
                                    <motion.text x="0" y="80" textAnchor="start" className="font-display" fontSize="130" letterSpacing="-0.04em" fontWeight="400" fill="none" stroke="#030035" strokeWidth="1.2" pathLength="1000" strokeDasharray="1000" style={{ strokeDashoffset: heroStrokeDashoffset, opacity: heroStrokeOp }}>
                                    ...
                                    <motion.text x="0" y="80" textAnchor="start" className="font-display" fontSize="130" letterSpacing="-0.04em" fontWeight="400" fill="#030035" stroke="none" style={{ opacity: heroFillOp }}>
                                      <tspan x="0" dy="0">{beats[0]?.title?.split(' ')[0]}</tspan>
                                      <tspan x="0" dy="117">{beats[0]?.title?.split(' ')[1]}</tspan>
                                      <tspan x="0" dy="117">{beats[0]?.title?.split(' ').slice(2, 5).join(' ')}</tspan>
                                      <tspan x="0" dy="117">{beats[0]?.title?.split(' ').slice(5, 7).join(' ')}</tspan>
                              ...
                              <>
                                <motion.div style={{ y: b1Y }} className="w-full relative max-w-[750px] xl:max-w-[900px] z-20 flex-shrink-0 hidden md:block">
                                  <svg viewBox="0 0 1000 820" className="w-full h-auto max-w-full max-h-[75vh] overflow-visible drop-shadow-sm">
                                    <motion.text 
                                      x="1000" y="100" textAnchor="end" className="font-display" fontSize="160" letterSpacing="-0.03em" fontWeight="400" 
                                      fill="none" stroke="#030035" strokeWidth="1.5" pathLength="1000" strokeDasharray="1000" 
                                      style={{ strokeDashoffset: b1StrokeDashoffset, opacity: b1StrokeOp }}
                                    >
                                    ...
                                    <motion.line 
                                      x1="600" y1="450" x2="1000" y2="450" stroke="#030035" strokeWidth="2.5" strokeDasharray="1000" pathLength="1000" 
                                      style={{ strokeDashoffset: b1StrokeDashoffset, opacity: b1StrokeOp }} 
                                    />
                                    <motion.text 
                                      x="1000" y="530" textAnchor="end" className="font-display" fontSize="95" letterSpacing="-0.02em" fontWeight="400" fill="none" stroke="#030035" strokeWidth="1" pathLength="1000" strokeDasharray="1000" style={{ strokeDashoffset: b1StrokeDashoffset, opacity: b1StrokeOp }}
                                    >
                                      <tspan x="1000" dy="0">Transformamos la deuda</tspan>
                                      <tspan x="1000" dy="110">en productividad.</tspan>
                                    </motion.text>
                                    <motion.text 
                                      x="1000" y="100" textAnchor="end" className="font-display" fontSize="160" letterSpacing="-0.03em" fontWeight="400" fill="#030035" stroke="none" style={{ opacity: b1FillOp }}
                                    >
                                      <tspan x="1000" dy="0">No somos un banco.</tspan>
                                    ...
                                    <motion.text 
                                      x="1000" y="530" textAnchor="end" className="font-display" fontSize="95" letterSpacing="-0.02em" fontWeight="400" fill="#030035" stroke="none" style={{ opacity: b1FillOp }}
                                    >
                                      <tspan x="1000" dy="0">Transformamos la deuda</tspan>
                                      <tspan x="1000" dy="110">en productividad.</tspan>
                                    </motion.text>
                                    <g className="cursor-pointer pointer-events-auto" onClick={() => window.location.href = beats[1]?.ctaPrimary?.to || '#'}>
                                      ...
                                      <motion.text x="590" y="753" textAnchor="middle" dominantBaseline="middle" className="font-sans" fontSize="18" fontWeight="600" fill="#f3f4f6" stroke="none" style={{ opacity: b1FillOp }}>{beats[1]?.ctaPrimary?.label}</motion.text>
                                    </g>
                                    <g className="cursor-pointer pointer-events-auto" onClick={() => window.location.href = beats[1]?.ctaSecondary?.to || '#'}>
                                      ...
                                      <motion.rect x="740" y="720" width="260" height="60" rx="30" fill="rgba(255,255,255,0.1)" stroke="none" style={{ opacity: b1FillOp }} />
                                      ...
                                      <motion.text x="870" y="753" textAnchor="middle" dominantBaseline="middle" className="font-sans" fontSize="18" fontWeight="600" fill="#030035" stroke="none" style={{ opacity: b1FillOp }}>{beats[1]?.ctaSecondary?.label}</motion.text>
                                    </g>
                                  </svg>
                                </motion.div>
                              ...
                              <>
                                <motion.div style={{ y: b2Y }} className="w-full relative mt-0 mb-0 -ml-1 hidden md:block">
                                  ...
                                  <svg viewBox="0 0 1000 360" className="w-full h-auto max-w-[1000px] max-h-[75vh] overflow-visible">
                                    ...
                                    <motion.text x="0" y="160" textAnchor="start" className="font-display" fontSize="115" letterSpacing="-0.04em" fontWeight="400" fill="#030035" stroke="none" style={{ opacity: b2FillOp }}>
                                      <tspan x="0" dy="0">Product pages that feel</tspan>
                                      <tspan x="0" dy="110">like one continuous motion.</tspan>
                                    </motion.text>
                                    <g className="cursor-pointer pointer-events-auto" onClick={() => window.location.href = beats[2]?.ctaPrimary?.to || '#'}>
                                      ...
                                      <motion.rect x="0" y="300" width="260" height="60" rx="30" fill="#030035" stroke="none" style={{ opacity: b2FillOp }} />
                                      ...
                                      <motion.text x="130" y="333" textAnchor="middle" dominantBaseline="middle" className="font-sans" fontSize="18" fontWeight="600" fill="#f3f4f6" stroke="none" style={{ opacity: b2FillOp }}>{beats[2]?.ctaPrimary?.label}</motion.text>
                                    </g>
                                    <g className="cursor-pointer pointer-events-auto" onClick={() => window.location.href = beats[2]?.ctaSecondary?.to || '#'}>
                                      ...
                                      <motion.rect x="280" y="300" width="260" height="60" rx="30" fill="rgba(255,255,255,0.1)" stroke="none" style={{ opacity: b2FillOp }} />
                                      ...
                                      <motion.text x="410" y="333" textAnchor="middle" dominantBaseline="middle" className="font-sans" fontSize="18" fontWeight="600" fill="#030035" stroke="none" style={{ opacity: b2FillOp }}>{beats[2]?.ctaSecondary?.label}</motion.text>
                                    </g>
                                  </svg>
                                </motion.div>
                            ...
                            {activeBeat > 2 && activeBeat !== 5 && <h1 className="font-display text-4xl leading-[0.92] tracking-[-0.04em] sm:text-5xl lg:text-7xl">{beats[activeBeat]?.title}</h1>}
                            {activeBeat > 2 && activeBeat !== 5 && <p className="mt-6 text-base leading-relaxed opacity-75 sm:text-lg max-w-md">{beats[activeBeat]?.description}</p>}
                            {beats[activeBeat]?.ctaPrimary && activeBeat > 2 && activeBeat !== 5 && (<div className={`mt-10 flex flex-wrap gap-4 w-full justify-start`}><Link to={beats[activeBeat]?.ctaPrimary.to}><motion.div style={{ backgroundColor: textColor, color: backgroundColor }} className="inline-flex items-center justify-center rounded-full px-10 py-4 text-base md:text-lg font-semibold transition-transform hover:-translate-y-0.5 shadow-lg">{beats[activeBeat]?.ctaPrimary.label}</motion.div></Link><Link to={beats[activeBeat]?.ctaSecondary.to}><motion.div style={{ borderColor: textColor }} className="inline-flex items-center justify-center rounded-full border px-10 py-4 text-base md:text-lg font-semibold backdrop-blur-xl transition-opacity hover:opacity-70 bg-white/10">{beats[activeBeat]?.ctaSecondary.label}</motion.div></Link></div>)}
                          </>
                        )}
                      </motion.div>
                    )}
