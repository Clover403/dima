...
function useIllustParallax() {
  ...
  const onMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    // Nilai jauh lebih besar karena SVG viewBox 200x200
    // Pakai attr: { transform } untuk SVG native attribute manipulation
    gsap.to(el.querySelectorAll('.ilbg'),  { 
      attr: { transform: `translate(${x*6}, ${y*4})` },
      duration: 0.6, ease: 'power2.out', overwrite: 'auto' 
    })
    gsap.to(el.querySelectorAll('.ilmid'), { 
      attr: { transform: `translate(${x*14}, ${y*10})` },
      duration: 0.5, ease: 'power2.out', overwrite: 'auto' 
    })
    gsap.to(el.querySelectorAll('.ilfg'),  { 
      attr: { transform: `translate(${x*22}, ${y*16})` },
  ...
  const onLeave = () => {
    const el = ref.current
    if (!el) return
    gsap.to(el.querySelectorAll('.ilbg, .ilmid, .ilfg'), {
      attr: { transform: 'translate(0, 0)' },
      duration: 1.0, ease: 'elastic.out(1, 0.45)', overwrite: 'auto'
    })
  }
...
export function IllustSimpleCredit({ className = '', opacity = 1, color = S }: IllustProps) {
  const { ref, onMove, onLeave } = useIllustParallax()
  
  return (
    <svg ref={ref} onMouseMove={onMove} onMouseLeave={onLeave} viewBox="0 0 200 200" fill="none" className={className} style={{ opacity }}>
      <EngravingGradients />
