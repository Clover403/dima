...
function AnimatedGrid() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      t += 0.04  // sedikit lebih cepat dari versi awal (0.008)
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          ctx.strokeStyle = `rgba(3,0,53,${0.08 + wave * 0.04})`
          ctx.lineWidth   = 0.5
...
export default function ProductsSection() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      sticky.querySelectorAll('.head-item'),
      { opacity: 0, y: 14 },
      {
        opacity: 1, y: 0,
  ...
  return (
    <div ref={wrapperRef} style={{ height: '350vh' }}>
      ...
      >
        ...
        {/* ── Header — lebih besar, lebih bawah ──────────────────────────── */}
        <div
          className="relative z-10 flex items-end justify-between px-10 md:px-16"
          style={{
            paddingTop: `${NAVBAR_H + 56}px`,   // ← lebih bawah dari navbar
            paddingBottom: '20px',
          }}
        >
          <div>
            <p className="head-item font-body text-[12px] tracking-[0.35em] uppercase text-navy mb-3">
              Productos
            </p>
            {/* ← font lebih besar */}
            <h2 className="head-item font-display text-[clamp(36px,4.5vw,64px)] text-navy leading-[1.1]">
              Soluciones con{' '}
              <em className="italic text-lightgray">ingeniería.</em>
            </h2>
          </div>

          <Link
            to="/productos"
            className="head-item group inline-flex items-center gap-2 font-body text-[11px] tracking-[0.25em] uppercase text-navy hover:text-lightgray transition-colors duration-300 border-b border-navy/25 hover:border-lightgray pb-0.5 mb-2"
          >
            Ver todos
        ...
        {/* ── Cards — fill remaining height after header ──────────────────── */}
        <div
          className="relative z-10 flex flex-col justify-center"
          style={{
            height: `calc(100vh - ${NAVBAR_H + 56}px - ${HEADER_H}px)`,
            paddingBottom: '24px',
          }}
        >
          <div
          ...
          >
            {products.map((product, i) => (
              ...
              >
                ...
                >
                  <h3 className="font-display text-white text-[19px] leading-snug">
                    {product.name}
                  </h3>
                  <p
                    className="font-body text-white/55 text-[13px] leading-relaxed overflow-hidden transition-all duration-500 ease-out"
                    style={{ maxHeight: 0, opacity: 0, marginTop: 0 }}
                    ref={(el) => {
                      if (!el) return
                      const card = el.closest<HTMLElement>('.group')
                      if (!card) return
            ...
            {/* CTA card */}
            <div
              className="product-card relative shrink-0 flex flex-col justify-end overflow-hidden"
              style={{
                width: '300px',
                height: 'clamp(360px, 58vh, 520px)',
                borderRadius: '20px',
            ...
            >
              <svg className="absolute top-6 right-6 opacity-[0.07]" width="52" height="52" viewBox="0 0 52 52" fill="none">
              ...
              <div className="w-7 h-px bg-bronze/40 mb-5" />
              <p className="font-display text-white text-[20px] leading-snug mb-7">
                Encuentra el producto{' '}
                <em className="text-bronze italic">ideal</em>{' '}
                para tu empresa.
              </p>
              <Link
                to="/productos"
                className="group inline-flex items-center gap-2 font-body text-[11px] tracking-[0.25em] uppercase text-bronze border border-bronze/35 px-5 py-3 transition-all duration-300 hover:bg-bronze hover:text-navy hover:border-bronze"
